...
import { apiUserGetCity, apiUserGetDeliveryAddress, apiUserGetDistrictByCity, apiUserGetWardByDistrict, apiUserUpdateDeliveryAddress } from "src/services/Address";
...
const UpdateDeliveryAddress = () => {
  ...
  useEffect(() => {
    apiUserGetDeliveryAddress().then((res) => {
      ...
      setWardId(res?.data?.data[0].wardId);
      setDetail(res?.data?.data[0].detail);
      setNameCity(res?.data?.data[0].city.name);
      setNameDistrict(res?.data?.data[0].district.name);
      setNameWard(res?.data?.data[0].ward.name);
    }).catch((err) => {
      console.log(err)
    })
  }, [call])
                  ...
                  <Grid item xs={12} md={6} sx={{ mb: 1 }} >
                    ...
                    <FormControl fullWidth>
                      <Autocomplete
                        ...
                        value={nameCity || null}
                        options={cities}
                        getOptionLabel={(option) => option['name'] || option} onChange={(e, newValue) => {
